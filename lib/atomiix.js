...
export default {
  ...
  start() {
    ...
    this.sendMessage(this.outSocket, setupOSCMessage(projectPath), this.console);
  },
  ...
  evaluate() {
    ...
    if (actions.audio.length > 0) {
      this.sendMessage(this.outSocket, atomiix.actionsToOSCBundle(actions.audio), this.console);
    }
  ...
  setupOSCListener(state, editorState, outSocket) {
    ...
    const sock = dgram.createSocket('udp4', msg => {
      ...
      try {
        ...
        if (actions.audio.length > 0) {
          this.sendMessage(outSocket, atomiix.actionsToOSCBundle(actions.audio));
        }
        if (actions.editor.length > 0) {
