...
export default {
  ...
  sendMessages(outSocket, messages) {
  ...
  setupOSCListener(state, editorState, outSocket) {
    ...
    const sock = dgram.createSocket('udp4', msg => {
      ...
      try {
        ...
        this.sendMessages(outSocket, atomiix.actionToOSC(actions.audio));
        handleActions(editorState, actions.editor);
      } catch (err) {
        console.log(err);
        this.console.logStderr(this.translation.errors.invalidOSC, err);
      }
    });
