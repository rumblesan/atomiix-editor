...
export default {
  ...
  start() {
    ...
    this.sendMessages(this.outSocket, [setupOSCMessage(projectPath)], this.console);
  },
  ...
  evaluate() {
    ...
    this.console.logStdout(this.translation.info.interpreting(firstLine + 1, lastLine + 1));
    const actions = atomiix.evaluate(this.atomiixState, text, firstLine);
    this.sendMessages(this.outSocket, atomiix.actionToOSC(actions.audio), this.console);
    handleActions(this.editorState, actions.editor);
  },

  freeAgent() {
    const {text, firstLine, lastLine} = getProgramText();
    this.console.logStdout(this.translation.info.interpreting(firstLine, lastLine));
    const actions = atomiix.free(this.atomiixState, text);
    this.sendMessages(this.outSocket, atomiix.actionToOSC(actions.audio), this.console);
    handleActions(this.editorState, actions.editor);
  },

  sendMessages(outSocket, messages, console) {
    ...
    messages.forEach(m => {
      ...
      outSocket.send(msg, 0, msg.length, port, host, () => {
        console.logStdout(this.translation.info.oscSent);
      });
    });
  },

  setupOSCListener(state, editorState, outSocket, console) {
    const inPort = atom.config.get('atomiix.incomingPort');

    const sock = dgram.createSocket('udp4', function(msg) {
      console.logStdout(this.translation.info.oscRecv);
      try {
        const incoming = atomiix.oscToAction(osc.fromBuffer(msg));
        const actions = atomiix.incomingAction(state, incoming);
        this.sendMessages(outSocket, atomiix.actionToOSC(actions.audio), console);
        handleActions(editorState, actions.editor);
      } catch (err) {
        console.logStderr(this.translation.errors.invalidOSC, err);
      }
    });
